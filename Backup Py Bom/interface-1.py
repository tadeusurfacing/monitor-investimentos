...
def iniciar_interface(df, cache):
    ...
    tk.Button(frame_botoes, text="💾 Salvar", command=lambda: salvar_dados(df),
              bg="#1f2937", fg="white", relief="flat", font=("Segoe UI", 10)).pack(fill="x", pady=5)

    tk.Button(frame_botoes, text="📄 Exportar PDF", command=lambda: exportar_pdf(df),
              bg="#1f2937", fg="white", relief="flat", font=("Segoe UI", 10)).pack(fill="x", pady=5)

    tk.Button(frame_botoes, text="🔄 Atualizar Cotações",
